...
--
-- Lets a user explicitly submit an existing draft paper trade intent
-- (public.trade_intents, status = 'draft' — see
-- 20260909000000_aoc_capital_signal_trade_intent_draft_handoff.sql) for Level
-- 1 risk policy evaluation. This is a separate, explicit user-confirmed
-- action, distinct from — and always after — draft creation:
--   - draft creation never runs the risk policy engine (unchanged)
--   - the system never automatically submits a draft
--   - the system never submits a draft immediately upon signal conversion
--   - submitting a draft is the only way it can ever be evaluated or become
--     a paper position
--
-- The RPC below re-runs the exact same 6 Level 1 rules as
-- evaluate_and_record_trade_intent() (20260903000000), against the exact same
-- symbol/side/quantity/notional_usd/leverage already stored on the draft row
-- — there is no client-suppliable override of any of those fields here,
-- consistent with the draft handoff's guarantee that they were derived
-- server-side from the originating signal. Submission (a human action),
-- the risk verdict (a Risk Constitution decision), and — if approved —
-- opening the resulting paper position all commit in one transaction, or
-- none of them do, following the atomic-audit pattern established in
-- 20260908000000_aoc_capital_atomic_audit_writes.sql. The submission event is
-- written before the verdict is known, so there is always independent audit
-- evidence that a human submitted the draft, distinct from whatever the Risk
-- Constitution subsequently decided.
--
-- Only a service-role client can call this RPC; RLS on trade_intents,
-- trade_decisions, paper_positions, and audit_ledger remains SELECT-only for
-- `authenticated`.
...
begin
  ...
  values (p_company_id, 'trade_intent_submitted_for_review', 'trade_intent', v_intent.id, p_actor,
    jsonb_build_object('symbol', v_intent.symbol, 'side', v_intent.side, 'quantity', v_intent.quantity, 'notionalUsd', v_intent.notional_usd));
...
revoke all on function public.submit_draft_trade_intent_for_review(text, uuid, uuid, text) from public;
grant execute on function public.submit_draft_trade_intent_for_review(text, uuid, uuid, text) to service_role;
