...
// (PR #18) — Safety — Static Source Checks ───────────────────────────────────
...
// rationale as tests/aoc-capital-allocation-exposure-safety.test.mjs). These
// tests statically inspect the service, API route, and page source to pin
// down that this report is read-only from a governance perspective: it never
// closes a position, never requests a close review, never marks a position
// to market, never refreshes valuation, never generates a signal, never
// creates/submits/cancels a draft trade intent, never runs Risk Constitution
// review, never creates a paper position, never mutates an audit record, and
// never references a broker/exchange/real-execution capability.

import { test } from "node:test";
import assert from "node:assert/strict";
import fs from "node:fs";

const serviceTs = fs.readFileSync("src/lib/capital/closed-position-performance-service.ts", "utf8");
const routeTs = fs.readFileSync("src/app/api/capital/performance/closed/route.ts", "utf8");
...
test("the service never calls a forbidden mutation helper function", () => {
  assert.doesNotMatch(
    serviceTs,
    /\brequestPaperPositionCloseReview\(|\bclosePaperPosition\(|\bmarkAllOpenPositions\(|\bmarkPositionToMarket\(|\brecordMarketPrice\(|\bgenerateSignals\(|\bcreateTradeIntent\(|\bselectStrategy\(|\bconvertSignalToDraftTradeIntent\(|\bcancelDraftTradeIntent\(|\bsubmitDraftTradeIntentForReview\(|\brecordAuditEvent\(/,
  );
});

test("the service never marks positions to market or closes positions — it reads paper_positions directly, never listPaperPositionsMarked/markAllOpenPositions/closePaperPosition/loadPortfolioOverview", () => {
  assert.doesNotMatch(serviceTs, /listPaperPositionsMarked|markAllOpenPositions|markPositionToMarket|closePaperPosition|loadPortfolioOverview/);
});
...
test("the service identifies closed positions by status, never by a client-suppliable filter", () => {
  assert.match(serviceTs, /p\.status === "closed"|status !== "closed"/);
});
...
test("the route requires auth and calls getClosedPositionPerformance, never a mutation service", () => {
  assert.match(routeTs, /requireAuthUser\(\)/);
  assert.match(routeTs, /getClosedPositionPerformance\(/);
...
test("the page never renders a Close Position or Request Close Review action", () => {
  ...
  assert.doesNotMatch(pageTsx, />\s*Trade Again\s*</i);
  assert.doesNotMatch(pageTsx, />\s*Reopen Position\s*</i);
});
...
test("no schema migration is added for this PR", () => {
  const migrationFiles = fs.readdirSync("supabase/migrations").filter((f) => f.includes("closed_position_performance") || f.includes("closed-position-performance"));
  assert.equal(migrationFiles.length, 0, "PR #18 is reporting-only and should not require a schema migration");
});
