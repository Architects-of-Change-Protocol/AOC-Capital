// ─── AOC Capital Signal Cohort Outcome Tracking v1 (PR #20) — Safety —
...
// rationale as tests/aoc-capital-strategy-performance-attribution-safety.
...
// creates/closes/marks a paper position, never requests a close review,
// never refreshes valuation, never mutates an audit record, never mutates a
...
const serviceTs = fs.readFileSync("src/lib/capital/signal-cohort-outcome-service.ts", "utf8");
const routeTs = fs.readFileSync("src/app/api/capital/performance/signals/route.ts", "utf8");
...
test("the service never infers a cohort or outcome from a symbol alone", () => {
  assert.doesNotMatch(serviceTs, /cohortKey.*symbol/i);
  assert.doesNotMatch(serviceTs, /deriveSignalCohortKey\([^)]*\bsymbol\b/);
});

test("the service only ever attributes a position/decision through a resolved trade-intent link, never by matching symbol", () => {
  assert.doesNotMatch(serviceTs, /positionByIntentId\.get\([^)]*symbol/i);
});
...
test("the route requires auth and calls getSignalCohortOutcomes, never a mutation service", () => {
  assert.match(routeTs, /requireAuthUser\(\)/);
  assert.match(routeTs, /getSignalCohortOutcomes\(/);
...
test("no schema migration is added for this PR", () => {
  const migrationFiles = fs.readdirSync("supabase/migrations").filter((f) => f.includes("signal_cohort") || f.includes("signal-cohort"));
  assert.equal(migrationFiles.length, 0, "PR #20 is reporting-only and should not require a schema migration");
});
